...
import com.github.ajalt.clikt.parameters.options.outputFile
import com.github.ajalt.clikt.parameters.options.validate
import com.github.ajalt.clikt.parameters.types.int
import io.github.tonyzhye.beancount.core.Employer
...
class BeanExampleCommand : CliktCommand(
    ...
    override fun run() {
        val options = ExampleOptions(
            ...
            dateBegin = beginDate?.let { LocalDate.parse(it) } ?: ExampleOptions().dateBegin,
            dateEnd = endDate?.let { LocalDate.parse(it) } ?: ExampleOptions().dateEnd
        )
        ...
        if (output != null) {
            output!!.writeText(ledger)
            echo("Example ledger written to ${output!!.absolutePath}")
        } else {
            echo(ledger)
        }
    }
